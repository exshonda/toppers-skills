...
LLMがTOPPERS/ASPカーネル向けのアプリケーションコードと `.cfg` を**素早く正しく**生成するための簡潔なリファレンス。詳細版（`rtos-spec.md`, `api-spec.md`, `static-api-spec.md`, `error-codes.md`）と併読する。

## 重要: ASPはμITRON 4.0ベースだが完全互換ではない

ASPカーネルは μITRON 4.0仕様をベースとしているが，**主要な点で仕様が異なる**。μITRON 4.0準拠の文献やコードをそのまま流用するとビルドエラーになる。詳細は `rtos-spec.md` の §1.1 を参照。要点：
...
- 概念・状態モデル・初期化／終了：`rtos-spec.md`
- C言語サービスコールの引数・返値・コンテキスト：`api-spec.md`
- `.cfg` の静的API書式・パラメータ：`static-api-spec.md`
- エラーコードの意味と発生条件：`error-codes.md`
