/*
 *  DataQueue Notify Program (Task → Task)
 *  スイッチスキャンタスクが psnd_dtq でLEDタスクに通知
 */
#include <kernel.h>
...
sw_task(intptr_t exinf){
    ...
    for(;;){
        ...
        if ((sw_state & SW2) != (tmp & SW2)) {
            SVC_PERROR(psnd_dtq(SW_DTQ, ((tmp & SW2) == SW2) ? SW2_ON : SW2_OFF));
        }
        if ((sw_state & SW3) != (tmp & SW3)) {
            SVC_PERROR(psnd_dtq(SW_DTQ, ((tmp & SW3) == SW3) ? SW3_ON : SW3_OFF));
        }
        if ((sw_state & SW4) != (tmp & SW4)) {
            SVC_PERROR(psnd_dtq(SW_DTQ, ((tmp & SW4) == SW4) ? SW4_ON : SW4_OFF));
        }
